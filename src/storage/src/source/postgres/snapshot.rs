...
pub(crate) fn render<G: Scope<Timestamp = MzOffset>>(
    ...
    // The key is an upstream table oid
    // The value is a BTreeMap of output_index: (table_desc, cast) since we can have multiple outputs
    // for a single table
    ...
    for output in source_outputs.into_iter() {
        ...
        if config.responsible_for(output.desc.oid) {
            reader_snapshot_table_info
                .entry(output.desc.oid)
                .or_insert_with(BTreeMap::new)
                .insert(output.output_index, (output.desc, output.casts));
        }
    }

    let reader_table_info = reader_snapshot_table_info.clone();
    let (button, transient_errors) = builder.build_fallible(move |caps| {
        ...
        Box::pin(SignaledFuture::new(busy_signal, async move {
                    ...
                    Err(PostgresError::PublicationMissing(publication)) => {
                        ...
                        for (oid, outputs) in reader_table_info.iter() {
                            ...
                            for output_index in outputs.keys() {
                                let update =
                                    ((*oid, *output_index, Err(err.clone().into())), MzOffset::from(u64::MAX), 1);
                    ...
                    match verify_schema(oid, expected_desc, &upstream_info, casts) {
                        Ok(()) => {
                            if table_name.is_none() {
                                table_name = Some((expected_desc.namespace.clone(), expected_desc.name.clone()));
                            }
                        ...
                        Err(err) => {
                            raw_handle
                                .give_fueled(
                                    &data_cap_set[0],
                                    ((oid, *output_index, Err(err.into())), MzOffset::minimum(), 1),
                                )
                                .await;
                            continue;
                        }
            ...
            // in the cluster.
            for outputs in reader_table_info.values() {
                for output_index in outputs.keys() {
                    trace!(%id, "timely-{worker_id} producing rewind request for output {output_index}");
    ...
    let snapshot_updates = raw_data
        ...
        .map(move |((oid, output_index, event), time, diff)| {
            let (_, casts) = &reader_snapshot_table_info
                .get(&oid)
                .and_then(|outputs| outputs.get(&output_index))
                .expect("table info should be filtered");
